...
<% if $MetroColumnNumber > 0 %>
    <div id="MetroStripUpper" $MetroFontSize>
    <% loop $MetroImageUpper %>
        <% if $First %>
            <div class="row one metroimage" style="clear:both">
            <ul class="{$Up.F4MetroListCount}">
        <% end_if %>
        <li style="margin:0px;position:relative">
                <a href="{$ImageLinkPage.AbsoluteLink}">
                    <div class="metrolink">$ImageLinkHTML</div>
                    <img class="metroradiusJJ" src="{$AbsoluteBaseURL}{$Image.FileName}" alt="$Image.Title"/>
